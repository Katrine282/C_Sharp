...
namespace LinqEksempler
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            elever.Add(new Elev("Oda", 7, 13));


            AlleKlasserOgEleverGroupByTrin();
            FoersteAarsKlasserLinq();
            Console.ReadLine();
        ...
        private static void FoersteAarsKlasserLinq()
        {
            ...
            IEnumerable<String> resultat = 
                klasser.Where(k => k.Klassetrin == 1)
                .OrderBy(k => k.Klassenavn)
                .Select(k => "Klasse " + k.Klassenavn + ", klassetrin:" + k.Klassetrin);
        ...
        private static void FoersteAarsKlasserLinqAnonymousObject()
        {
            Console.WriteLine("FoersteAarsKlasserLinqAnonymousObject");
            var resultat = klasser.Where(k => k.Klassetrin == 1)
                .OrderBy(k => k.Klassenavn)
                .Select(k => new { navn = k.Klassenavn, klassetrin = k.Klassetrin} );
            foreach (var k in resultat)
            {
                Console.WriteLine(k.navn + " " + k.klassetrin);
            }
            foreach (var k in resultat)
            {
                Console.WriteLine(k.navn + " " + k.klassetrin);
        ...
        private static void AlleKlasserOgEleverGroupByTrin()
        {
            ...
            {
                System.Console.WriteLine("Key (klassetrin):" + k.Key + ", antal:" + k.Count());
        ...
        private static void AlleKlasserOgEleverGroupByTrinMax()
        {
            ...
            foreach (var k in grupperetresultat)
            {
                System.Console.WriteLine("Key (klassetrin):" + k.Key + " " + "max:" + k.Max(ke => ke.Alder));
            }
            //eller sådan:
            //var grupperetresultat = resultat.GroupBy(ke => ke.Klassetrin).Select(g => new { g.Key, maxalder = g.Max(ke => ke.Alder) });
            //foreach (var k in grupperetresultat)
            //{
            //    System.Console.WriteLine(k.Key + " " + k.maxalder);
            //}
        }
